...
class ToolsPanel(Gtk.Box):
    ...
    def create_overlays_button(self):
        """Create the toggle overlays button"""
        container = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)

        button = Gtk.Button(label="Toggle Overlays...")
        button.add_css_class("action-button")
        button.set_hexpand(True)
        button.set_valign(Gtk.Align.CENTER)
        button.set_tooltip_text("Open the overlays dialog to toggle groups of remaps on or off")
        button.connect('clicked', self.on_toggle_overlays)

        container.append(button)

        # Add help button
        if self.parent_window:
            help_title = "Toggle Overlays"
            help_text = (
                "Opens a dialog for toggling groups of macOS-style remaps on or off.\n\n"
                "Use this to:\n"
                "• Disable Mac-style shortcuts in specific app categories\n"
                "  (browsers, file managers, VSCode, etc.)\n"
                "• Switch to a minimal configuration with only terminal handling\n"
                "• Enable user-defined custom overlays\n\n"
            ...
            help_button.set_tooltip_text("Show help for overlays")
            help_button.add_css_class("tools-help-button")
            help_button.connect('clicked', lambda btn: self.show_help_dialog(help_title, help_text))
            container.append(help_button)
    ...
    def on_toggle_overlays(self, button):
        """Handle toggle overlays button click"""
        if not self.parent_window:
            debug("Toggle Overlays requested but no parent window available")
            return

        from toshy_gui.gui.overlays_dialog_gtk4 import OverlaysDialog
        dialog = OverlaysDialog(parent=self.parent_window, cnfg=self.cnfg)
        dialog.present()
