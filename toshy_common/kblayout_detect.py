...
def select_backend(desktop_env, session_type):
    ...
    Choice keys primarily off the desktop environment, because KDE, GNOME, and
    Cinnamon expose layout state through focus-independent channels that work on
    both X11 and Wayland. The generic XKB reader is the fallback for any other
    X11 desktop, and a generic Wayland reader for other compositors. Backends are
