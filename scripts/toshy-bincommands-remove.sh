...
rm -f "${LOCAL_BIN}/toshy-kblayout-check"
rm -f "${LOCAL_BIN}/toshy-sshot-check"
rm -f "${LOCAL_BIN}/toshy-spotlight-check"
rm -f "${LOCAL_BIN}/toshy-kwin-dbus-service"
rm -f "${LOCAL_BIN}/toshy-cosmic-dbus-service"
rm -f "${LOCAL_BIN}/toshy-wlroots-dbus-service"

# Deprecated command names: no longer installed by the setup script, but
# keep removing leftovers from older installs for a while. (Deprecated 2026-07)
...
echo "- toshy-sshot-check"
echo "- toshy-spotlight-check"
