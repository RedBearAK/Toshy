...
Toshy is available as a Nix flake with a NixOS module and a Home Manager module. This is the recommended installation method for NixOS.

### Flake input

Add the Toshy flake to your `flake.nix` inputs:
...
Import the module and enable the service in your NixOS configuration:
...
- Installs the Toshy package and xwaykeyz keymapper
- Creates five systemd user services (config, session-monitor, kwin-dbus, wlroots-dbus, cosmic-dbus)
- Creates a tray icon service when `gui.enable = true`
...
> **Note:** With Home Manager standalone (no NixOS module), add your user to the `input` group at the system level: `users.users.alice.extraGroups = [ "input" ];`
...
### Keyboard type on NixOS

If your keyboard is misidentified, edit `~/.config/toshy/toshy_config.py` and add your device to the `keyboards_UserCustom_dct` dictionary:

```python
keyboards_UserCustom_dct = {
    'My Keyboard Device Name': 'Apple',  # or 'Windows', 'IBM', 'Chromebook'
}
```

Then restart the config service:
...
Build Toshy or xwaykeyz without the module:
...
# Provides pkgs.toshy and pkgs.xwaykeyz
```
